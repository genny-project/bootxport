...
public class BatchLoading {
  ...
  public Map<String, DataType> dataType(
    ...
    project.entrySet().stream().filter(d -> !d.getKey().matches("\\s*")).forEach(data -> {
      ...
      String code =
          ((String) dataType.get("code")).replaceAll("^\"|\"$", "");;
      String name =
          ((String) dataType.get("name")).replaceAll("^\"|\"$", "");;
    ...
    project.entrySet().stream().forEach(data -> {
      ...
      try {
        ...
        try {
          ...
          try {
            existing = service.findQuestionQuestionByCode(parentCode,
                targetCode);
            if (existing == null) {
          ...
          try {
            if (isSynchronise()) {
              QBaseMSGMessageTemplate val =
                  service.findTemplateByCode(templateObj.getCode(),
                      "hidden");
              if (val != null) {
                val.setRealm("genny");
                service.updateRealm(val);
